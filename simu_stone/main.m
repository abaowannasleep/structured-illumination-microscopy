...
for i = 1:length(focus)
    ...
    bmodfr(:, :, i) = fourier_mod(bfringes{i,pshift});
    bmodmr(:, :, i) = moer_mod(bfringes{i,pshift}, T);
...
%%
figure(1)
plot(bzfr(510,:), 'b*', LineWidth=0.5);  
hold on;
plot(bzmr(510,:), 'm*', LineWidth=0.5); 
hold on;
plot(bzps(510,:), 'r*', LineWidth=0.5);
hold off
ylim([0.2 0.8]);
xlim([100,800]);
...
x = 100:900;
y = 510*ones(size(x));
plot3(x, y, bzps(510,100:900)+0.01, 'r', LineWidth=2.5);  
hold off 
zlim([zmin zmax]);xlim tight, ylim tight;
view(-20, 80)
title('(a)', 'FontName','Times New Roman','FontSize',26)
...
x = 100:900;
y = 510*ones(size(x));
plot3(x, y, bzmr(510,100:900)+0.01, 'm', LineWidth=2.5);  
hold off 
zlim([zmin zmax]);xlim tight, ylim tight;
view(-20, 80)
title('(b)', 'FontName','Times New Roman','FontSize',26)
xlabel('x/pixel');ylabel('y/pixel');zlabel('height/mm');
set(gca,'FontSize',18);
...
x = 100:900;
y = 510*ones(size(x));
plot3(x, y, bzfr(510,100:900)+0.01, 'b', LineWidth=2.5);  
hold off 
zlim([zmin zmax]);xlim tight, ylim tight;
view(-20, 80)
title('(c)', 'FontName','Times New Roman','FontSize',26) 
xlabel('x/pixel');ylabel('y/pixel');zlabel('height/mm');
set(gca,'FontSize',18);
cb = colorbar;
...
zlim([zmin zmax]);xlim tight, ylim tight;
view(-20, 80)
title('(a)', 'FontName','Times New Roman','FontSize',28)
xlabel('x/pixel');ylabel('y/pixel');zlabel('height/mm');
set(gca,'FontSize',18);
ax5 = nexttile; mesh(fzmr); 
zlim([zmin zmax]);xlim tight, ylim tight;
view(-20, 80)
title('(b)', 'FontName','Times New Roman','FontSize',28)
xlabel('x/pixel');ylabel('y/pixel');zlabel('height/mm');
set(gca,'FontSize',18);
ax6 = nexttile; mesh(fzfr); 
zlim([zmin zmax]);xlim tight, ylim tight;
view(-20, 80)
title('(c)', 'FontName','Times New Roman','FontSize',28)
xlabel('x/pixel');ylabel('y/pixel');zlabel('height/mm');
set(gca,'FontSize',18);
cb = colorbar;
cb.Label.FontSize = 18;
